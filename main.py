...
def calculate_probabilities():
    try:
        ...
        adjusted_home_goals = ((values["entry_home_scored"] + values["entry_xg_home"] * 0.5 - values["entry_away_conceded"]) *
                                ...
                                (values["entry_injuries_home"] * 0.05) - (values["entry_yellow_cards_home"] * 0.03) + league_adjustment))
        ...
        adjusted_away_goals = ((values["entry_away_scored"] + values["entry_xg_away"] * 0.5 - values["entry_home_conceded"]) *
                                ...
                                (values["entry_injuries_away"] * 0.05) - (values["entry_yellow_cards_away"] * 0.03) - league_adjustment))
        ...
        if lay_bets:
            ...
            if edge > 0:
                ...
                result_label["text"] = (
                    f"Recommended Lay Bet: {best_lay}\n"
                    f"Fair Odds: {lay_bets[best_lay]:.2f}\n"
                    f"Bookmaker Odds: {bookmaker_odds:.2f}\n"
                    f"Edge: {edge:.4f}\n"
                    f"Kelly Stake (%): {kelly_stake_fraction * 100:.2f}%\n"
                    f"Lay Stake: £{lay_stake:.2f}\n"
                    f"Liability: £{liability:.2f}"
                )
            else:
                result_label["text"] = "No suitable Lay bets found (No positive edge)."
        else:
